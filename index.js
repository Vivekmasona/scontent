...
// Middleware for parsing JSON body and handling CORS
app.use(cors());
...
// Changed Route to POST Method
app.post("/extract", async (req, res) => {
  // Reading URL from POST JSON Body
  let { url } = req.body;

  if (!url) {
    return res.status(400).json({ status: "error", message: "Valid URL required in JSON body" });
  }
  ...
  try {
    ...
    return res.status(500).json({ status: "error", message: "Extraction failed", results: [] });
  }
});
